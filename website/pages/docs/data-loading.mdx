...
## Large datasets: level of detail

When zoomed far out, many bars share one pixel column, and drawing each of them
paints the same pixels over and over. The chart's level of detail, on by
default, draws one OHLC-preserving stick per device-pixel column instead, so a
frame costs the width of the plot rather than the number of bars in view: 200,000
candles and their volume fitted into a 944 px plot paint about 2,900 marks,
against 400,000 bar by bar.

```ts
const chart = createChart(el, {
  timeScale: { minBarSpacing: 0.01 }, // allow zooming out past one bar per pixel
  conflationFactor: 1,                // default; 2 to 4 merge into wider columns
});
...
It engages only below about one CSS px per bar (exactly one at a whole device
pixel ratio). The default time scale never zooms out past one bar per CSS px
(`minBarSpacing: 1`), so a chart that keeps that floor draws exactly as before;
lower the floor to show long histories whole.

Each stick is **OHLC-preserving**: open comes from the first bar in the column,
close from the last, high and low from the extremes, volume is summed, and the
stick takes the colours of the bar that closed it. Line series keep each
column's first, lowest, highest and last values, an HLC area adds each run's
highest high and lowest low so its band and gaps match the full frame, and
histograms keep their lowest and highest bar, which cover the pixels every bar
would have. A custom chart type is
always drawn in full, even one registered under a built-in name. The step is transparent to your data layer: indicators,
autoscale and the crosshair see every bar.

Fixed-size merge helpers are also available for manual use:
...
The level of detail reduces rendering work for zoomed-out data. It does not
reduce the memory retained by the source series or indicator outputs. Pair it
with a history window for sustained sessions.
