...
When the instrument's tick changes with price, one constant step cannot serve both
sides of a boundary: 0.05 folds the cent levels below it into rows the book does not
have, and 0.01 above it offers rows the venue forbids. Pass the instrument's
`TickSchedule` instead, as `tickSchedule` or to `buildRows` in place of the tick size.
Every row is then a price its own band allows, and `groupBy` counts ticks of that band,
stopping at a boundary rather than labelling a row past it. With a schedule, `groupBy`
counts whole ticks: a fraction rounds down, so 2.5 groups two ticks into each row. The
constant path multiplies instead, so there 2.5 makes rows 2.5 ticks apart, between
the prices the instrument trades at.
...
const bandedLadder = new DomLadder({ tickSize: ticks.minMove, tickSchedule: ticks, groupBy: 1 });
const bandedRows = buildRows(depth, ticks, 5); // 0.05 rows below 100, 0.25 rows from 100
```

An `Instrument` with `tickBands` carries the same schedule as `instrument.tickSchedule`;
see [Instrument metadata](/docs/instruments#price-dependent-ticks).
