...
primitives. The price pane is pane 0, unless a chart built with `movablePrimaryPane` lets
it move below its studies (`primaryPaneIndex()` says where); pane 1, 2, and so on are created
automatically when you pass `paneIndex: 1` (or higher) to `addSeries`. All panes share the same time
...
The chart's level of detail: an OHLC-preserving step that engages when bars become narrower
than about one CSS px at high zoom-out. The bars sharing a device-pixel column are drawn as
one stick: open = first open, close = last close, high = max high, low = min low, volume =
sum. It never averages prices, and data, indicators and autoscale still see every bar. On by
default; `ChartOptions.conflate: false` turns it off.
...
place it behind or above the series. The library includes the OpenAlgo vector
glyph and shows chart-owned corner branding by default. Hosts can replace or disable it.
The separate text watermark is opt-in and disabled by default.

See also: [Branding and watermarks](/docs/branding-and-watermarks),
...
`TradingOrder` and `TradingPosition` records, decoding each order row with `decodeOrder`.
