...
It follows a change of ratio too: a window dragged to a screen of another density, or a zoom. The chart watches a resolution media query for the ratio in force, made again after each change, and the window's `resize`, and re-sizes and repaints every canvas at once. At fractional ratios such as 1.25 and 1.5, common on laptop screens, each boundary between panes is rounded onto a device pixel and the separator is one device pixel tall, so no pane is stretched by a fraction of a pixel. Where the browser reports a canvas's device-pixel box (Chromium, Firefox), the canvas takes exactly that many pixels.
