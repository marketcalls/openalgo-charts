...
| `paneIndex` | A non-negative integer, counted with the price pane first (see below). Absent means pane 0, the price pane; a bogus one is a rejection, because a drawing parked on a pane that does not exist is invisible and unfindable. |
...
- **A drawing copied beside the candles pastes beside the candles.** The payload counts
  panes with the price pane first and the study panes after it in order, whatever slot the
  price pane held where the drawing was copied. A chart built with
  [`movablePrimaryPane`](/docs/scales-and-panes#moving-the-price-pane-below-its-studies) can
  keep its price pane below its studies, and a paste there, or on another chart that keeps it
  on top, still lands on the price pane. On a chart with the price pane on top, which is every
  chart without the option, the count is the chart's own slots, as it always was.
- **A pane the receiving chart does not have is folded onto one it does**, counted the same
  way, so a drawing from a taller stack lands on the last study pane, never on the price pane.
  Adding a primitive creates the pane it names, so without this a drawing copied out of an
  indicator pane would conjure an empty pane in a single-pane chart.
