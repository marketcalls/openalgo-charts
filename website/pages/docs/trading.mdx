...
above shows the raw pointer price; with a tick schedule, preview
`ticks.round(price)` instead, which is the price `requestModify` will send. The last
...
schedule's `minMove`. Without `tickSchedule`, snapping is exactly the constant
`tickSize` path above.
...
The snapped price is an exact decimal, and a price written exactly halfway between two
valid prices rounds up. Price limits are checked after snapping, as with a constant tick,
