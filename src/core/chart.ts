...
import {
  compactVolume, type ZoomAnchor, type DoubleClickAction, type DoubleClickEvent, type ChartWatermarkOptions,
  ...
  type PointerSample, type PointerInfo, type ChartEventClick, type ChartDragEvent,
  type ChartDragEndEvent, type ContextMenuTarget, type LayoutSetter,
  type LayoutChangeEvent, type ContextMenuEvent, type RendererFallbackEvent,
} from './chart-types';
...
export class Chart {
  ...
  private _addPane(weight = 1): Pane {
    ...
    this._pixels._observeCanvases(pane, true);
    return pane;
  }
  ...
  public removePane(index: number, options: IndicatorEditOptions = {}): boolean {
    ...
    this._pixels._observeCanvases(pane, false);
    pane.destroy();
  ...
  private readonly _onPointerMove = (e: PointerEvent): void => {
    ...
    if (this._axisDrag === 'time') {
      this._motion._beginAutoscaleMotion();
  ...
  private readonly _onWheel = (e: WheelEvent): void => {
    ...
    const remaining = this._motion._zoomGlide === null ? 0 : this._motion._zoomGlide.totalLogFactor - this._motion._zoomGlideApplied;
    ...
    if (!this._animZoom || !ZoomGlide.shouldAnimate(logFactor)) {
      this._motion._applyZoom(focusX, logFactor);
      return;
    }
    const lead = logFactor * ZoomGlide.leadFraction();
    const epoch = this._motion._navigationEpoch;
    this._motion._applyZoom(focusX, lead);
    if (this._destroyed || epoch !== this._motion._navigationEpoch) return;
    this._motion._startZoomGlide(focusX, logFactor - lead);
  };
