...
- **base canvas**: background, grid, series, indicator lines, normal-layer primitives, and the axis strips with their tags. Repainted on `Light`/`Full`.
- **top canvas**: crosshair, hover highlights, primitives being dragged (order lines), magnet. Repainted on `Cursor`, without touching the base canvas.
