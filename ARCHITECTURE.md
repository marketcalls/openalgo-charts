...
We split the package into **eight loadable tiers** so the base stays tiny and heavy features are opt-in (dynamic `import()` / separate entry points). This keeps the base engine at 74.00 KB Brotli while supporting 102 indicators, 51 drawing tools, 15 chart types, footprint/TPO/orderflow, a GPU render backend and, in the eighth tier, the chrome itself (§8.5). Measured sizes for every tier are in the README size budget.
