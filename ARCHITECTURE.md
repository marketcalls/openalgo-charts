...
We split the package into **seven loadable tiers** so the base stays tiny and heavy features are opt-in (dynamic `import()` / separate entry points). This keeps the base engine at 66.41 KB Brotli while supporting 102 indicators, 51 drawing tools, footprint/TPO/orderflow and a GPU render backend. Measured sizes for every tier are in the README size budget.
