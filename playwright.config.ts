...
export default defineConfig({
  ...
  projects: [
    ...
    { name: 'chromium', use: { ...devices['Desktop Chrome'] }, testIgnore: /(?:yfinance(?:-(?:ui-253|mobile|templates|indicator-source|routed-study|grid|pane-reorder|tick-schedule|viewport|watchlist-news|account|study-policies))?|widget-grid|widget-ui-253|widget-data-loading|widget-localization|widget-alerts|chart-data-export|instruments|indicator-source-markers|alert-line-drag|table-layout|date-navigation|tick-schedule-drag|drawing-viewport|widget-watchlist-news|widget-account|icon-raster|stack-order|native-input-points)\.spec\.ts/ },
    ...
    ...(['chromium', 'firefox', 'webkit'] as const).map(browserName => ({
      ...
      testMatch: /yfinance-(?:ui-253|mobile|templates|indicator-source|grid|routed-study|pane-reorder|tick-schedule|viewport|watchlist-news|account|study-policies)\.spec\.ts/,
