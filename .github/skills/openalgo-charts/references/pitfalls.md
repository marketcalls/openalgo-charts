...
**A longer retained history makes every live tick slower, even with the same bars in view.** Each frame that carries a tick recomputes every study over the whole history (no built-in implements `calcTail` yet), and a data write repaints every pane. The recorded endurance workload (two charts, 150 bars in view, five studies each, ten updates a second) measured a frame-interval p95 of 17 ms at 2,000 bars per chart and 717 ms at 50,000 on one desktop machine; see `docs/browser-endurance.md`. Bound the retained history to the longest lookback users need, and measure on the target device.
