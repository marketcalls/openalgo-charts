...
**Slot 0 is not the price pane once a user moves it.** A host that passes `0` to mean "the price pane" (an order line, a volume overlay, a price alert check, a coordinate call for a DOM overlay) lands on a study pane after `setPrimaryPaneIndex` or `movePane`. Omit the pane, which defaults to the price pane, or ask `chart.primaryPaneIndex()`.
