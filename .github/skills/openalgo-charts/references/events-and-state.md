...
| `priceAxisMoved` | `{ paneIndex, from, to }` | `movePriceAxis` succeeded: a pane's prices and their scale changed strip. Re-read `priceAxisState` for any menu still open on that axis. `movePriceAxis` is deprecated and goes in 3.0.0; `setPriceAxisPlacement` emits `priceAxisPlacementChanged` instead. |
