...
| `supertrend` | `(bars, period = 10, multiplier = 3) => SupertrendPoint[]` | `{ value, direction }`; `value` is `NaN` during ATR warmup and on a bar with no ATR, after which the bands carry on. `direction` `-1` = uptrend, `+1` = downtrend. |
